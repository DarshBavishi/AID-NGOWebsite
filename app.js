...
app.post("/pay", async (req, res) => {
  ...
  try {
    ...
    ejs.renderFile(
      ...
      { name: req.body.name, amount: orderAmount / 100 },
      (err, data) => {
        if (err) {
          console.log(err);
          res.send({ error: err });
...
app.post(
  "/causes&stories/:storyid/readmore/comment",
  isLoggedIn,
  function (req, res) {
      ...
      Product.findById(req.params.id, function (err, foundProduct) {
        if (err) {
          ...
          for (var i = 0; i < founduser.cart.products.length; i++) {
            if (
              founduser.cart.products[i]._id.toString() ==
          ...
          if (index !== -1) {
            founduser.cart.total =
              founduser.cart.total - founduser.cart.products[index].cost;
...
app.post("/order", isLoggedIn, function (req, res) {
  ...
  var id = req.user.orders[req.user.orders.length - 1]._id
    .toString()
    .substring(0, 8);
      ...
      ejs.renderFile(
        ...
        (err, data) => {
          if (err) {
            ...
            pdf
              .create(data, options)
              .toFile("invoice12.pdf", function (err, data) {
                if (err) {
                  console.log(err);
                } else {
                  console.log("File created successfully");
